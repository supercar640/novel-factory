...
scene 모드에서는 장면 단위로 작성→검토를 반복하며, PD가 수동으로 병합하여 에피소드를 완성한다.

**상태 전이 흐름:**
```
WRITING → save(sc001.md) → next → SCENE_DECISION
  → approve  → scene_count++ → WRITING (다음 장면)
  → revise   → 마지막 장면 제거 → WRITING (수정)
  → reject   → 마지막 장면 제거 → WRITING (재작성)
  → merge-episode → 5,500자 검증 → 병합 → WRITING_DECISION
```

**AI가 할 일 (장면 작성):**
1. 다음을 모두 참조하여 **하나의 장면**을 작성한다:
   - `context/` 폴더의 6개 컨텍스트 파일
   - `context/foreshadow.md` (복선 파일이 있으면 회수 검토)
   - 선정된 전개 방향 (`status`의 selected_developments)
   - 문체 레퍼런스 (설정된 경우)
   - 기존 승인된 장면들의 내용 (연속성 유지)
2. 장면을 `sc{NNN}.md` 파일명으로 저장한다:
   ```bash
   python nfc.py save manuscript "drafts/sc001.md"
   ```
3. 다음 단계로 이동한다:
   ```bash
   python nfc.py next
   ```
   → `SCENE_DECISION` 단계로 자동 전이
4. 사용자에게 장면을 제시하고 의사결정을 요청한다:
   ```
   [A]pprove(승인) / [M]odify(수정) / [D]ismiss(폐기) / merge-episode / scenes
   ```

**사용자 응답 처리 (SCENE_DECISION):**
- `A`: `python nfc.py approve` → scene_count 증가, 다음 장면 작성으로 이동 (WRITING)
- `M "<피드백>"`: `python nfc.py revise "<수정 피드백>"` → 마지막 장면만 제거, 피드백 반영 후 재작성
- `D`: `python nfc.py reject` → 마지막 장면만 제거, WRITING으로 복귀하여 재작성
- `scenes`: 승인된 장면 목록과 글자 수 확인 (상태 변경 없음):
  ...
  === 장면 목록 ===
    ...
    누적: 3,221자 / 5,500자 기준
  ```
- `merge-episode`: 승인된 장면들을 하나의 에피소드로 병합
  - **5,500자 게이트**: 누적 글자 수 < 5,500이면 병합 차단, 장면을 더 작성해야 함
  - 병합 성공 시 `WRITING_DECISION` 단계로 전이

**사용자 응답 처리 (WRITING_DECISION, 병합 후):**
- `A`: `python nfc.py approve` → Phase 4(퇴고)로 이동
- `M "<피드백>"`: `python nfc.py revise "<수정 피드백>"` → scene_count 리셋, WRITING으로 복귀하여 처음부터 재작성
- `D`: `python nfc.py reject` → scene_count 리셋, WRITING으로 복귀하여 처음부터 재작성
- `pd-proofread <file>`: AI 퇴고 생략, 컨텍스트 갱신으로 직행
