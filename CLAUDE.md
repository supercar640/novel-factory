...
*작가실 모드 (draft-room, v2.9)*: 가변 토폴로지 작가실(writers' room) 릴레이 집필
- 단계 구성·LLM 배정·프롬프트가 **토폴로지(topology) × 크루(crew) × 역할(role)** 3축으로 분리돼 PD가 회차별로 조합을 바꾼다. 프리셋은 `nf/presets/draft_room/`(빌트인) → `projects/{title}/draft_room/`(프로젝트 우선)로 해석.
...
2. **조율(`mode: live`) 및 최종 승인은 Claude Code(라이브) + PD**: 첫 live 단계부터(이후 전부)는 자동 실행하지 않고 안내만 출력 → Claude Code가 직전 산출물을 읽어 해당 역할(예: 톤조율→정합성감사→교정)을 순서대로 수행하고 `ep###_making/`에 새 파일로 저장.
   - `balanced`는 조율(tone/audit/copy)을 live로 둬 PD 검수 체크포인트를 보존. `all-*`/`cheap`은 전 단계 자동(라이브 없음).
- 회차 번호는 `episode_count+1` 자동. 모든 단계가 `episodes/ep###_making/`에 **새 파일로 보존**됨 (덮어쓰기 없음).
- 재미 보존 점검: `draft-room` 출력이 회차별 stem에 맞춰 `fun-diff <직전 자동본> <최종본>` 명령을 안내.
- 조회/스캐폴드: `room-list`(토폴로지·크루) / `room-roles`(역할, 출처 표시) / `room-show <topo> <crew>`(합성 미리보기) / `room-init`(프로젝트에 프리셋 복사).
- `--gemini / --codex` 는 **deprecated**(발상=`--gemini`, 증폭=`--codex` override로 자동 변환). 다음 버전 제거 예정 → `--crew`/`--override` 사용.
...
| `draft-room [--topology <t>] [--crew <c>] [--override <k=v>]` | v2.9: 가변 토폴로지 작가실 릴레이 (토폴로지×크루×역할 합성, auto 자동·live 안내) |
| `room-list` | v2.9: 작가실 토폴로지·크루 나열 (빌트인+프로젝트) |
| `room-roles` | v2.9: 작가실 역할 나열 (출처 표시) |
| `room-show <topo> <crew> [--override <k=v>]` | v2.9: 토폴로지+크루 합성 결과 미리보기 |
| `room-init` | v2.9: 프로젝트에 draft_room/ 프리셋 스캐폴드 복사 |
...
├── draft_room/          # (선택) 작가실 프리셋 오버라이드 — room-init로 스캐폴드, 빌트인보다 우선
