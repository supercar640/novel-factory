...
### v2.8 작가실 모드 (draft-room)

| 명령어 | 설명 |
|--------|------|
| `draft-room [--gemini <type>] [--codex <type>]` | 6에이전트 작가실 릴레이 (발상·증폭 자동, 조율 라이브) |

기존 "막 갈김" 초고가 성의 없어지는 문제를 "발상 밀도" 평가 기준으로 전환하고, 과감함을 끝까지 살리기 위해 증폭과 조율 릴레이를 거치는 **deluxe 모드**다 (기존 파이프라인과 별개 운영). 6개의 에이전트가 역할을 나누어 집필한다:

| 구간 | 단계 | 담당 | 역할 | 산출물 |
|------|------|------|------|--------|
| 자동 | G1 광인 | **Gemini** | 장르 관습 박살, 안전한 전개 금지, 발상 밀도·예측 불가능성 우선 | `01_g1_chaos.md` |
| 자동 | G2 씨앗채굴자 | **Gemini** | 광기에서 이야기 씨앗을 골라 캐릭터 동기·감정 훅으로 결속 | `02_g2_seed.md` |
| 자동 | C1 판돈러 | **Codex** | 절대 깎지 않고 'Yes, and—' 방식으로 판돈·위기·스케일 증폭 | `03_c1_stakes.md` |
| 자동 | C2 디테일변태 | **Codex** | 결정적 대사·감각 묘사·연출 과장 디테일링 (명장면 제조) | `04_c2_detail.md` |
| 라이브 | K1 톤조율 | **Claude Code** | 폭주 에너지를 톤·리듬에 맞게 깎되 재미 보존 | `05_k1_tone.md` |
| 라이브 | K2 정합성감사 | **Claude Code** | 플롯·캐릭터·복선 충돌 검수 및 수선 | `06_k2_audit.md` |
| 라이브 | K3 교정 | **Claude Code** | 맞춤법·오탈자·띄어쓰기·비문 라인 레벨 일소 (맨 마지막) | `07_k3_copyedit.md` |

G1~C2(자동 구간)는 NF가 외부 CLI(기본 gemini-cli, codex-cli)를 호출해 연속 실행하고, K1~K3(라이브 구간) 및 최종 승인은 Claude Code와 PD가 수행한다. 산출물은 `episodes/ep###_making/`에 전 단계가 덮어쓰기 없이 보존된다. 조율 과정에서 재미 요소가 깎였는지 확인하려면 `fun-diff 04_c2_detail.md 07_k3_copyedit.md` 명령으로 검출할 수 있다.
